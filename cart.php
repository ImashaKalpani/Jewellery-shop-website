...
if (isset($_POST['update_cart'])){
    ...
    $qty = $_POST['qty'];
    $QUTY = filter_var($qty,FILTER_SANITIZE_STRING);

    $update_qty = $conn->prepare("UPDATE 'cart' SET qty =? WHERE id = ?" );
    $update_qty->execute([$qty,$cart_id]);

    $success_msg[] = 'cart quantity update successfullly';
    
}
// delete item from wishlist
if(isset($_POST ['delete_item'])){
    $cart_id = $_POST['cart_id'];
    $cart_id =filter_var($cart_id,FILTER_SANITIZE_STAING);

    $varify_delete_item = $cone->prepare("SELECT * FROM 'cart' WHERE id =?");
    $varify_delete_item->execute([$wishlist_id]);

    if($varify_delete_items->rowCount()>0){
        $delete_cart_id=$cone->prepare("DELETE FROM 'wishlist' WHERE id =?");
        $delete_cart_id->execute([$cart_id]);
        $success_msg[]="cart item delete successfuly";
    }else{
...
if (isset($_POST['empty_cart'])){
    ...
    if ($varify_empty_item->rowCount() > 0){
        ...
        $success_msg[]="empty item delete successfuly";
    }else{
        $warning_msg[] = 'cart item already delete';
    }
...
<head>
    ...
    <title>Jewlery Shop - wishlist page</title>
</head>
...
<body>
    ...
    <div class="main">
        ...
        <div class="title2">
            <a href="home.php">home </a><span>/ cart</details></span>
        </div>
            ...
            <div class ="box-container">
                ...
                if ($select_cart->rowCount()>0){
                    while($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)){
                        $select_products =$cone->prepare("SELECT * FROM 'products' WHERE id =?");
                        $select_products->execute([$fetch_cart['product_id']]);
                        if ($select_products->rowCount()>0){
                            $fetch_products = $select_products->fetch(PDO::FETCH_ASSOC)
                            ?>
                            <from method="post" action ="" class="box">
                                <input type="hidden" name="cart-id" value= "<?=$fetch_cart['id'];?>">
